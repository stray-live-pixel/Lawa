...
Инструмент не является привилегированной оболочкой. Дочерний `cwd` должен
оставаться внутри workspace родителя. Файлы workflow и task-file разрешены только
в этом workspace или в read-only папке родительского run; managed restrictions и
режим approvals не ослабляются.
