...
`/office` — отдельная страница того же UI с общей темой Gravity UI. Сначала в
комнате только Босс. Кнопка смартфона открывает чат: введите цель и рабочую папку.
«Закрепить цель» сохраняет дословный pin и **сразу запускает Босса** через работающий
`lawa serve`. ID заказа остаётся в `/office?run=<run-id>`.
...
Все обсуждения, поручения и ответы проходят через общий чат. Доступны только
Босс (`boss`), Разработчик (`developer`) и Чел (`human`).
...
- Первый ход Босса начинается сразу. После этого каждый сотрудник проверяет чат
  каждые **5 минут своего бездействия**, считая от завершения собственного хода.
  При пустой очереди модель не запускается; сообщения во время работы ждут следующего хода.
...
команды; для нового поведения создайте цель в телефоне. `resume/reply` отклоняются
для командного заказа, чтобы не запустить второго Босса.

HTTP: `GET/POST /api/teams` — список/создание; `GET /api/teams/{run}` — чат;
