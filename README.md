...
Если совместимый UI для того же `--root` уже запущен, в том числе через локальный
`lawa serve`, команда только откроет вкладку. Иначе она создаст общий сервер на
свободном порту `127.0.0.1`. Другой root обслуживается отдельно; занятый порт сам
по себе не считается работающим UI Lawa.

Создавшая сервер команда после завершения workflow или серии остаётся в режиме
просмотра до Ctrl+C: можно получить финальное состояние или загрузить страницу
впервые. Координатор и его блокировки уже освобождены. Команды, использующие готовый
сервер, завершаются после выполнения и не останавливают его. Ctrl+C в команде-владельце
закрывает общий UI для всех вкладок; другие процессы workflow продолжают работу.

Для запуска без открытия UI и для автоматизации с выходом сразу после выполнения
добавьте `--no-ui`. Уже работающий сервер этот флаг не останавливает:
...
отключает открытие UI всей серии. После остановки UI результат остаётся в хранилище;
его можно посмотреть через `lawa serve --root <то же хранилище>`.
...
Отдельная команда `lawa review --prompt "Проведи ревью изменений" --cwd /path/to/project`
проводит Code Review с сохранённым контекстом и доказательными экскурсиями по коду.
По умолчанию открывается UI `/code-review`; `--no-ui` явно отключает его.
Модели трёх этапов задаются до запуска. Подробности, конфигурация и контракт
результатов — в [руководстве Code Review](docs/code-review.md).
...
`.ui.lock` содержит адрес общего локального UI и удерживается его процессом через
`flock`. При одновременном старте остальные процессы ждут HTTP-готовности владельца;
после его выхода старая запись адреса не мешает новому запуску. Файл нельзя удалять
при работающем UI: блокировка привязана к inode. Ручной `serve` на внешнем интерфейсе
не участвует в локальном обнаружении UI.
