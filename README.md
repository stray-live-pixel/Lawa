...
`PATH` нужен только legacy-командам `run`, `resume` и повторяющимся сериям.
...
flowchart LR
    ...
    V -->|create_thread| A[Задача A в Codex App]
    V -->|create_thread| B[Задача B в Codex App]
    V -->|create_thread| C[Задача C в Codex App]
    A --> S[Итоговый кубик]
    B --> S
    C --> S
...
Для каждого готового кубика управляющий чат одним app-вызовом создаст отдельную
задачу Codex App с уникальным title и первым сообщением, затем сохранит её ID и
будет ждать через App API. Поэтому задача сразу видна в интерфейсе, получает живые
сообщения и остаётся доступной для общения. Уникальный title позволяет после сбоя
найти уже созданную задачу, а не запускать дубль. Модель выбирается по приоритету:
...
Страница показывает раскрываемое дерево родительских и дочерних workflow, кубики,
их статусы, память, ссылки в Codex и VS Code, а также готовый UML PNG. Состояние
обновляется каждые три секунды. Повреждение одного run отображается отдельно и не
скрывает остальные процессы. Сервер не запускает и не изменяет workflow.
...
`run` поддерживает три взаимоисключающих режима. Каждый повтор создаёт обычный
run с отдельными `runId`, снимком workflow, статусом и историей:

```sh
# Следующий run сразу после успешного завершения предыдущего.
lawa run workflow.json <обычные-флаги> --repeat immediate --max-runs 10

# Первый run сразу, последующие — через час после завершения предыдущего.
lawa run workflow.json <обычные-флаги> --repeat after --repeat-delay 1h

# Ежедневно в 10:00 по Москве.
lawa run workflow.json <обычные-флаги> \
...
и не создаёт следующий run; проблемный обычный run можно разбирать и продолжать
отдельной командой `resume`, но это не возобновляет серию автоматически.
...
`~/.light-ai-workflows/series/<series-id>/series.json`: режим, число начатых и
завершённых run, текущий `runId`, ближайшее время и последнюю ошибку. Stop-маркер
...
Desktop; app-native driver покрыт автоматическими тестами без расхода модели.
