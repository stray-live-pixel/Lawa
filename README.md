...
Это первый слой личностей и диалога. Автоматический цикл руководства всеми
стадиями, закрытие заказа и отдельный UI комнаты ещё не реализованы.
После дочерней работы для следующего решения Босса может понадобиться `reply`.
...
Прямой адрес `/office` открывает отдельную сцену: пластилиновый изометрический
офис в бежевых тонах с растениями и Боссом за MacBook по центру. На странице
сцена, кнопка «Босс», кнопка смартфона и общий переключатель темы Lawa.
Кнопка справа от головы показывает точку статуса: серую в ожидании, синюю
при работе и зелёную при мониторинге. Нажатие переключает эти три демо-состояния;
над головой появляется короткая реплика, а в ожидании облачко исчезает.
В старом интерфейсе ссылки на эту страницу нет.
Статус и реплики над Боссом пока демонстрационные. Чат в смартфоне работает
с реальными сохранёнными заказами. Комната и персонаж — PNG в HTML/CSS-вёрстке;
[изображения и промпты](ui/src/assets/office/README.md) хранятся в репозитории.

### Общая база знаний команды

Кнопка смартфона в офисе открывает чат. Выберите существующий заказ или введите
новую цель и папку проекта. «Закрепить цель» сохраняет заказ с ожидающим Боссом,
но не запускает Codex. Для запуска используйте `lawa resume <run-id> --root <путь>`;
ID выбранного заказа находится в адресе `/office?run=<run-id>`.

Цель закреплена над перепиской и сохраняется дословно, без лимита в 50 слов.
Сообщения содержат ID автора, серверную дату UTC и текст от 1 до 50 слов
(разделители — Unicode-пробелы, дополнительный предел — 8 КБ). Интерфейс показывает
имя, аватарку по ID участника, локальную дату и bubble. Из UI пишет только Чел.

Каждому кубику обоих форматов, включая Босса и продолжения, доступны
`team_read {}` и `team_post {"text":"Важный для команды факт"}`. Автор привязан
координатором к личности/кубику; инструмент не принимает чужой ID автора.
Новый prompt просит читать общую цель перед работой и важными решениями и
публиковать только важные факты, решения, результаты и препятствия.
Чат не будит завершившего ход агента автоматически и не заменяет `lawa reply`.

Хранилище — `<root>/<корневой-run-id>/team.json`: pin, реестр участников и сообщения.
Все дочерние workflow находят тот же корень через `parentRunId`; разные заказы
изолированы. ID личности включает её run, поэтому одноимённые исполнители разных
дочерних запусков не сливаются. Личная память и память кубиков сохраняются отдельно.
`team.lock` сериализует короткие записи между процессами независимо от блокировки
координатора; `fsync` и атомарное переименование защищают целостность снимка.
Повтор с тем же ID и содержимым возвращает прежнее сообщение. Старые run без
`team.json` читаются без миграции; первый пост сохраняет их общую базу.

HTTP: `GET/POST /api/teams` — список/создание заказа,
`GET /api/teams/{run}` — чат, `POST /api/teams/{run}/messages` — реплика Чела
(`id`, `text`). Запись требует same-origin; это локальный интерфейс без аккаунтов.
Телефон обновляет открытый чат каждые три секунды. Автосводки, полнотекстового
поиска, push-уведомлений и автоматического выполнения сообщений пока нет.
