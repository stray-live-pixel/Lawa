...
безопасного действия, последнюю активность и краткую ошибку. `logs` читает
приватный нормализованный `events.jsonl`. В него входят lifecycle процесса,
thread/turn, безопасный тип item, финальный agent message, ошибка и числовые
счётчики token usage.
...
run, PID/turn/активность и текущие безопасные действия кубиков, память, события,
тикеты, папку run и UML. Dashboard не читает внутренние каталоги Codex и не
отдаёт raw rollout.
