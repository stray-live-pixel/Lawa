...
Для каждого готового кубика Lawa создаст отдельную задачу Codex. Модель выбирается
по приоритету: `model` кубика, затем общий `model` workflow, затем конфигурация
Codex. Отсутствующие `effort` и `speed` наследуются из Codex.
...
MVP реализован: доступны `validate`, `run`, `resume`, `serve`, установка скилла, параллельные
