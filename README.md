...
`arm64`. Для установки нужны `curl` или `wget`. Основной `/lawa` запускается
в Codex App с task-инструментами; установленный и авторизованный Codex CLI в
`PATH` нужен только legacy-командам `run` и `resume` из внешнего терминала.
Повторяющиеся серии из Codex App также используют живые task-инструменты.
...
- в legacy CLI адресно прерывает активные turn при `Ctrl+C` или `SIGTERM`.

Lawa не заменяет Codex и не вызывает модель напрямую. Инструменты, авторизация и
ограничения задаются в Codex. Workflow может выбрать общую модель, а отдельный
кубик — переопределить `model`, `effort` и `speed`. Если значение не задано ни
на одном доступном уровне, оно наследуется из Codex. Lawa отвечает за граф,
состояние и координацию.
...
flowchart LR
    ...
    I -->|lawa app-run / app-next| V[Состояние и зависимости Lawa]
    V -->|create_thread или fork_thread| A[Задача A в Codex App]
    V -->|create_thread или fork_thread| B[Задача B в Codex App]
    V -->|create_thread или fork_thread| C[Задача C в Codex App]
    A --> S[Итоговый кубик]
    B --> S
    C --> S
...
3. Агент проверит JSON, создаст run и через встроенные инструменты Codex App
   запустит дочерние задачи в той же рабочей папке.
...
Для каждого готового кубика управляющий чат создаст отдельную задачу Codex App,
сохранит её ID и явно переместит в верхнеуровневый раздел Tasks. Сохранённый проект
использует `create_thread`; временный checkout без projectId — `fork_thread` с тем
же рабочим каталогом. Поэтому агент сразу виден в интерфейсе, отдаёт живые сообщения
и остаётся доступным для общения независимо от того, сохранён ли cwd как проект.
Уникальный title позволяет после сбоя найти уже созданную задачу, а не запускать
дубль. Модель выбирается по приоритету:
`model` кубика, затем общий `model` workflow, затем конфигурация
Codex. Отсутствующий `effort` наследуется из Codex. Явный `speed` пока доступен
только legacy CLI: task API Codex App не принимает service tier, и app-native
режим отклоняет такую настройку вместо молчаливого изменения поведения.

## Формат workflow

Workflow — один JSON-объект с непустым `id` и массивом `steps`. Необязательный
корневой `model` задаёт общую модель для кубиков, которые не выбрали собственную.
Если общего `model` тоже нет, модель наследуется из Codex. У каждого кубика
...
Каждое поле можно задавать независимо. `model` кубика переопределяет общий
`model` workflow; отсутствующие `effort` и `speed` наследуются из Codex. Явный
`normal` отключает унаследованный Fast mode для этого кубика. Lawa не хардкодит
меняющийся каталог моделей и их effort: несовместимое сочетание отклоняет Codex,
а Lawa показывает его ошибку без подстановки другого режима.
...
```text
lawa app-run <workflow.json> --cwd <проект> \
  (--task <текст> | --task-file <путь>) \
  --initiator-thread-id <id> [--parent-run <run-id>] [--repeat ...]
...
# Legacy только для внешнего терминала: отдельные app-server без живых событий App.
...
наблюдения: она не меняет зависимости кубиков или правила исполнения.

### Локальное дерево workflow

```sh
lawa serve
```

Команда запускает read-only dashboard по адресу
[`http://127.0.0.1:60800`](http://127.0.0.1:60800). Она читает сохранённые run из
`~/.light-ai-workflows`, поэтому видит процессы других экземпляров Lawa и не требует
подключения к Codex. Другой root или адрес задаются через `--root` и `--listen`.
При публикации не на loopback Lawa печатает предупреждение.

Страница группирует корневые workflow в секции «В работе», «Сломавшиеся» и
«Успешные»; две завершённые секции по умолчанию свёрнуты. Внутри доступны дерево
дочерних workflow, кубики, память и кнопки Codex, папки run в VS Code и готового
UML PNG. Технический статус, run ID и время обновления вынесены в отдельную
мелкую строку, чтобы не смешиваться с названием workflow.

Над деревом доступны строка поиска и временное окно. Поиск регистронезависимо
проверяет вхождение в названия workflow и кубиков, постановку, память, тикет,
run/thread ID, cwd и остальные сохранённые поля. Совпадение в дочернем workflow
показывает целое дерево с родителем. По умолчанию выбраны последние 24 часа;
доступны окна от часа до 30 дней и «За всё время».

Номера страниц переключают соседние окна выбранной длины, а не ограничивают число
карточек. Например, для периода «За последний час» страница 2 показывает час перед
текущим, страница 3 — ещё один час раньше. Все элементы выбранного окна выводятся
сразу. Незавершённые деревья всегда видны в текущем окне и не повторяются на
исторических страницах. Период, поиск и страница сохраняются в URL и при автообновлении.

Если сохранённая постановка содержит `ISSUE_ID`/`TICKET_ID`, название и явный
HTTP(S)-URL тикета (в отдельных полях либо в `TRACKER_CONTEXT`), dashboard
показывает тикет рядом с соответствующим workflow. Произвольные ссылки из
описания не угадываются и не становятся кнопкой тикета.

Для известного UUID задачи Codex кнопка «Тред JSONL» открывает в VS Code полный
сырой rollout из `$CODEX_HOME/sessions` (по умолчанию `~/.codex/sessions`) или
соседнего `archived_sessions`.
Dashboard не отдаёт содержимое этого приватного файла по HTTP. Локальный layout
rollout — деталь текущей установки Codex, а не стабильный API: если Codex изменит
имена или расположение файлов, необязательная кнопка исчезнет, но дерево продолжит
работать. Состояние обновляется каждые три секунды. Повреждение одного run
отображается отдельно и не скрывает остальные процессы. Сервер не запускает и не
изменяет workflow.

Встроенный preview доступен на
[`http://127.0.0.1:60800/preview`](http://127.0.0.1:60800/preview). Он использует
тот же production-шаблон, но сложные фейковые данные, поэтому внешний вид можно
оценить без единого настоящего run.

HTML, CSS, JavaScript и preview встроены в Go-бинарник. Node.js, npm, отдельные
статические файлы и дополнительные runtime-зависимости не нужны.

### Повторяющиеся серии

`app-run` поддерживает три взаимоисключающих режима. Каждый повтор создаёт обычный
app-native run с отдельными `runId`, снимком workflow, статусом и живыми задачами:

```sh
# Следующий run сразу после успешного завершения предыдущего.
lawa app-run workflow.json <обычные-флаги> --repeat immediate --max-runs 10

# Первый run сразу, последующие — через час после завершения предыдущего.
lawa app-run workflow.json <обычные-флаги> --repeat after --repeat-delay 1h

# Ежедневно в 10:00 по Москве.
lawa app-run workflow.json <обычные-флаги> \
  --repeat cron --cron "0 10 * * *" --timezone Europe/Moscow
```

Cron использует пять стандартных полей: `minute hour day-of-month month day-of-week`.
Допустимы числа, `*`, диапазоны, списки и шаг `/`; секунды не указываются. Временная
зона обязательна и задаётся именем IANA. Если календарная точка наступила во время
активного run, она пропускается: после завершения выбирается первая будущая точка.

`immediate` и `after` без `--max-runs` продолжаются до явной остановки. Положительный
лимит `N` создаёт ровно `N` run. Два run одной серии не работают одновременно.
Управляющая задача создаёт heartbeat Codex App. Между пробуждениями короткая
`app-series-next` заново открывает серию под lock, возвращает уже существующий run
либо создаёт ровно один новый. Поэтому серия не зависит от фонового shell-процесса
и переживает завершение turn или перезапуск приложения, сохраняя живой UI.
...
`series-stop` атомарно запрещает будущие run. Уже работающий run не прерывается и
спокойно завершается, после чего серия получает состояние `stopped`. Политика после
`failed` или `interrupted` — `stop-on-failure`: серия получает состояние `failed`
через `app-series-fail` и не создаёт следующий run; проблемные задачи остаются
видимы и доступны для разбора в Codex App.
...
Серия хранит небольшой отдельный снимок в
`~/.light-ai-workflows/series/<series-id>/series.json`: driver, режим, число
начатых и завершённых run, текущий `runId`, ближайшее время и последнюю ошибку.
App-native серия рядом хранит приватный неизменяемый `app-template.json`, из
которого heartbeat создаёт следующий run без исходного процесса. Stop-маркер
пишется отдельно, чтобы команда остановки не могла потерять конкурентное обновление.
...
Legacy `resume` открывает тот же run, сверяет сохранённые чаты и запускает только новые
готовые кубики. Если последний turn был прерван, Lawa отправляет один `continue` в
тот же чат с общей моделью workflow и настройками кубика из сохранённого
`workflow.json`. Упавший кубик пользователь продолжает вручную; успешно завершённый
не перезапускается.
...
internal/appdriver/   устойчивый протокол задач, которыми владеет Codex App
internal/codex/       клиент Codex App Server
internal/dashboard/   встроенный локальный сервер, дерево run и HTML preview
...
MVP реализован: доступны app-native задачи Codex App, `validate`, legacy `run`/`resume`,
`serve`, установка скилла, параллельные
...
Legacy-интеграция проверена живыми пятикубиковыми сценариями на macOS с Codex
Desktop; app-native driver, повторяющиеся серии, восстановление interrupted turn и
маршруты сохранённого/временного cwd покрыты автоматическими тестами без расхода модели.
