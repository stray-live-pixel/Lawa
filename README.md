...
На macOS бинарник при установке и обновлении создаёт **Lawa** с фирменной
иконкой в `/Applications` (список приложений macOS). Это служебный `.app`-launcher,
а не отдельная поставляемая программа: UI и сервер остаются в одном Go-бинарнике.
Иконка открывает **https://local.lawa.app:60800** в браузере по умолчанию;
`launchd` запускает сервер при необходимости, повторное нажатие открывает уже работающий UI.
Установка запрашивает права администратора для локального адреса в `/etc/hosts`
и доверия сертификату только для `local.lawa.app`. `--yes` не отменяет системный диалог.
Окно без панелей зависит от браузера и не гарантируется.

Восстановить интеграцию после отказа в правах: `lawa desktop-install`.
Если порт занят прежним HTTP-сервером, сначала остановите его; установщик не
завершает чужие процессы. Лог: `~/Library/Application Support/Lawa/desktop/server.log`.
CLI сохраняется при ошибке настройки macOS, но установщик завершается с ошибкой.
Ключ хранится локально с правами `0600`; сертификат действует 365 дней. Повторная
установка сохраняет его либо обновляет за 30 дней до истечения. Если Lawa долго
не обновлялась, `lawa desktop-install` обновит сертификат с системным запросом прав.
...
[http://127.0.0.1:60800](http://127.0.0.1:60800). После установки macOS-интеграции
этот порт использует HTTPS: открывайте `https://local.lawa.app:60800` через иконку
или `lawa ui`. Сервер уже запущен через launchd; повторный `lawa serve` не нужен.
Явный другой `--listen` сохраняет прежний HTTP-режим. Новые workflow находятся сверху
