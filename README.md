...
`skipped`-ветки, лимит и причину завершения. `logs` читает безопасное для терминала
...
attempt, решение и объяснение, выбранный переход, skipped-ключи, техническая
...
состояния каждого указанного источника — как `succeeded`, так и `failed` — и не
является бизнес-условием. Кубик с `decisions` получает память и безопасную
...
workflow как `succeeded` или `failed`. Не выбранные ключи показываются как
`skipped`. Внутри одного plan порядок fan-out берётся из `to`, а порядок готовых
`after` — из `steps`; сохранённая история не переигрывается при `resume`.
