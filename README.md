...
возвращает прежний `runId`, а не создаёт дубль.

Инструмент не является привилегированной оболочкой. Дочерний `cwd` должен совпадать
с workspace родителя; нужную подпапку агент указывает в задаче и открывает уже
внутри разрешённого workspace. Файлы workflow и task-file читаются через безопасный
корень только из workspace или read-only папки родительского run; managed
restrictions и режим approvals не ослабляются.
...
сверху. Компактная строка показывает прогресс `готово/всего` и только выполняемые
шаги; раскрытие показывает все шаги. Клик по шагу открывает обновляемую боковую
панель с сохранёнными сообщениями, командами и их выводом. Также доступны память,
события, тикеты, папка run и UML. Dashboard не читает внутренние каталоги Codex,
не показывает raw reasoning и не отдаёт raw rollout.
