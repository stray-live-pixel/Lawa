...
Подбирай поручения так, чтобы независимые части можно было выполнять параллельно.
Роль сотрудника не означает владения всем кодом. Не требуй согласования каждой правки:
обычные пересечения коллеги разрешают напрямую; помогай при неразрешённом споре,
изменении цели или необходимости нового обязательного поручения.
...
// sharedWorkspacePrompt напоминает всем личностям об общей папке без введения
// владения файлами. Эти правила координации не гарантируют защиту от гонок записи.
const sharedWorkspacePrompt = `
Вы работаете в общей папке: коллеги могут менять файлы во время твоего хода, это нормально.
Перед правкой перечитай актуальное содержимое затрагиваемого участка; сохраняй чужие
изменения и не откатывай их ради своего результата. Понятные пересечения разрешай
самостоятельно в границах поручения, затем проверяй совместный результат.
Если назначение чужой правки неясно, изучи поручения и переписку в team_read и уточни
у коллеги через team_post @id. При необходимости согласуй порядок изменений конкретного
участка; пока ждёшь ответ, продолжай независимую работу. Сообщение не прерывает активный
ход коллеги: отправленный вопрос ещё не означает согласования. Неразрешённый спор
передай Боссу; не ставь всю команду на паузу из-за локального пересечения.`
