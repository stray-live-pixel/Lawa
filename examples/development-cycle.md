...
Владелец разрешил публикацию изображений только в GitHub. QA использует `gh api`
для загрузки оригиналов и копий с разметкой в отдельную ветку
`qa-artifacts/issue-<номер>-<runId>-<visitId>` репозитория задачи, затем встраивает
изображения по SHA коммита материалов в комментарий issue через `gh issue comment
--body-file`. Пошаговый контракт находится в шаблоне отчёта QA и входит в prompt.
...
не меняют; QA отдельно разрешены коммиты материалов через GitHub API по правилам
